...
function QuestionSingleDesktop({
  currentQuestion,
  userAnswers,
  handleAnswerChange,
  showFeedback,
  ...
  return (
    <div className="w-full h-full max-w-[1600px] mx-auto flex flex-row gap-6 overflow-hidden">
      {/* Cột trái: Nội dung câu hỏi */}
      <div className="flex-1 min-w-0 bg-white rounded-xl p-8 border border-slate-200 flex flex-col shadow-sm h-full overflow-y-auto custom-scrollbar">
      ...
      {/* Cột phải: Giải thích */}
      {showFeedback && (
        <div className="w-[45%] shrink-0 bg-white border border-slate-200 rounded-xl p-6 h-full overflow-y-auto shadow-sm animate-fade-in custom-scrollbar">
...
export default QuestionSingleDesktop;